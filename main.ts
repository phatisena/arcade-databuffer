...
namespace DataBuffer {
    ...
    export function decodeText(bufv: Buffer) {
        ...
        for (let i = 1; i < bufv.length; i++) {
            if (bytelen > 0) {
                ...
                bytesum = (bytesum > 0) ? bytesum * bytemax : bytemax, bytelen--
            } else {
                bytelen = bufv[i], strtxt += String.fromCharCode(byteval)
                byteval = 0, bytesum = 0
            }
        }
        return strtxt
    ...
    export function encodeImage(img: Image) {
        ...
        for (let i = 1;i < (img.width * img.height);i++) {
            ...
            if (img.getPixel(xi, yi) == numv) {
                ...
                bytelen = bitcalc(numc, 8), numarrv.push(bytelen)
                while (bytelen > 0) {
                    numarrv.push(numc % bytemax)
                    numc = Math.floor(numc / bytemax)
                    bytelen--
                }
                numarrv.push(numv)
                numv = img.getPixel(xi, yi)
                numc = 1
            }
        }
        bytelen = bitcalc(numc, 8), numarrv.push(bytelen)
    ...
    export function decodeImage(bufv: Buffer) {
        let i = 0, ii = 0, bytemax = 8 ** 2 * 4, bytelen = bufv[i], byteval = 0, bytesum = 0, w = 0, h = 0, img: Image, imgRowBuffer: Buffer
        while (bytelen > 0) {
            i++
            if (bytesum > 0) byteval += bufv[i] * bytesum
            else byteval += bufv[i]
        ...
        while (bytelen > 0) {
            ...
            bytesum = (bytesum > 0) ? bytesum * bytemax : bytemax
            bytelen-- 
        } i++, h = byteval, img = image.create(w, h), imgRowBuffer = pins.createBuffer(h)
        bytelen = bufv[i], byteval = 0, bytesum = 0
        while (bytelen > 0) { i++
            if (bytesum > 0) byteval += bufv[i] * bytesum
            else byteval += bufv[i]
            bytesum = (bytesum > 0) ? bytesum * bytemax : bytemax
